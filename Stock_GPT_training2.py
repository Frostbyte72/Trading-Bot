    ...
    for ticker in tickers['Symbol']:
        ...
        if update:
            ...
            try:
                ...
                print('Trying')
                get_data(ticker)
                ticker_data = query("SELECT * FROM DAILY WHERE Symbol = '{}' ORDER BY DATE ASC".format(ticker))
        ...
        ticker_data , ticker_target = prepare_data(ticker_data)
        ...
        ticker_target = ticker_target.iloc[0:-time_step,] #accounr for reshaping take of last timstep records from the targets

        print(ticker_target)
        target = pd.concat([target,ticker_target])
        print(target)
        if count == 1:

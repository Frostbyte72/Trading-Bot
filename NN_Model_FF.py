...
def create_model(shape,loss_function,optimiser,learning_rate,layer_size,activation,dropout):
    ...
    model.add(layers.Dense(layer_size, input_shape=(shape,), name = 'input')) #setsup first hidden layer aswell as defing the input layers shape
    model.add(layers.Dropout(rate=dropout)) 
    model.add(layers.Dense(layer_size, activation=activation, name = 'Layer2'))
    ...
    model.add(layers.Dense(layer_size,activation = activation, name='Layer3')) #another hidden layer
...
def main(symbol,explain = False,plot= False,epochs = 150, batch_size =10,time_step = 7,loss_function = 'mse',optimiser = 'adam',learning_rate = 0.001,layer_size = 32, activation = 'tanh',dropout = 0.1):
    ...
    #Convert the dataset (datafrmae) to a matrix (Numpy ndarray)
    x = dataset.to_numpy()
    ...
    model = create_model(columns,loss_function,optimiser,learning_rate,layer_size,activation,dropout)
    ...
    if plot:
        ...
        plt.title('Feed Forward Model Predictions for {}'.format(symbol))
        plt.legend()
        plt.show()
...
if __name__ == '__main__':
    main('GOOGL',plot = True,epochs = 150,batch_size = 10,layer_size = 64,activation = 'tanh')
